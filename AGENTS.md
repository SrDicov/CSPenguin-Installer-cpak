...
- `install.sh` — host installer: deps → downloads → Wine prefix → runtime/patches → CSP install → desktop integration. Source of truth for versions and prefix layout.
...
- `install.sh` is interactive with 10s `/dev/tty` timeouts: all menus default to cancel on timeout. Non-interactive agents should pass an explicit flag (`--update`, `--update-wine`) or expect cancel. In cpak mode (`CPAK_CONTAINER_ID` set or `CSPENGUIN_CPAK=1`) the CSP version prompt is skipped, defaulting to choice 1 unless `CSPENGUIN_CSP_VERSION` is set.
...
- `*.AppImage` (`opencode-*`) is git-ignored local tooling, not part of the package. Ignore it.
